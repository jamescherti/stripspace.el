...
- Customizations: *whitespace-cleanup-mode* uses the built-in `whitespace-cleanup` function (not all users prefer `whitespace-cleanup` because it deletes more than just trailing whitespace). There is no way to change this function in *whitespace-cleanup-mode*. On the other hand, the *stripspace* package defaults to the built-in `delete-trailing-whitespace` function, but users can assign a different function by setting `stripspace-clean-function`. For example, setting `stripspace-clean-function` to `whitespace-cleanup` makes *stripspace* behave like `whitespace-cleanup-mode`.
